...
GENETIC_OPERATORS_WEIGHT = [CROSSOVER_PROB, MUTATION_PROB, 1 - (CROSSOVER_PROB + MUTATION_PROB)]
...
def get_string_candidate():
    string_candidate = [chr(i) for i in range(65, 65 + 26)]
...
def mutation(agents, evaluations, string_candidate):
    ...
    next_agent = [i if random.random() > 0.1 else get_sentence(string_candidate, 1)[0] for i in selectd_agent]
    return next_agent
...
if __name__ == '__main__':
    ...
    agents = [['H', 'E', 'I', 'S', 'E', 'I'] for i in range(AGENT_NUM)]
